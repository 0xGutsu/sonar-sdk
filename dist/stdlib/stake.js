...
function add_stake_(owner, amount, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        ownership_cap = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
        yield add_stake_with_cap_(ownership_cap, yield Coin.withdraw_(owner, $.copy(amount), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), $c);
        return;
    });
}
...
function add_stake_with_cap_(owner_cap, coins, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        ...
        validator_set = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), new aptos_1.HexString("0x1"));
        temp$1 = yield find_validator_((validator_set).active_validators, $.copy(pool_address), $c);
        if (yield Option.is_some_(temp$1, $c, [move_to_ts_2.AtomicTypeTag.U64])) {
            temp$3 = true;
        }
        else {
            temp$2 = yield find_validator_((validator_set).pending_active, $.copy(pool_address), $c);
            temp$3 = yield Option.is_some_(temp$2, $c, [move_to_ts_2.AtomicTypeTag.U64]);
        }
        if (temp$3) {
            yield update_voting_power_increase_($.copy(amount), $c);
        }
        else {
        }
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        if (yield is_current_epoch_validator_($.copy(pool_address), $c)) {
            yield Coin.merge_((stake_pool).pending_active, coins, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        }
        else {
            yield Coin.merge_((stake_pool).active, coins, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        }
        temp$4 = yield Staking_config.get_($c);
        [, maximum_stake] = yield Staking_config.get_required_stake_(temp$4, $c);
        voting_power = yield get_next_epoch_voting_power_(stake_pool, $c);
        if (!($.copy(voting_power)).le($.copy(maximum_stake))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.ESTAKE_EXCEEDS_MAX), $c));
        }
        yield Event.emit_event_((stake_pool).add_stake_events, new AddStakeEvent({ pool_address: $.copy(pool_address), amount_added: $.copy(amount) }, new move_to_ts_2.SimpleStructTag(AddStakeEvent)), $c, [new move_to_ts_2.SimpleStructTag(AddStakeEvent)]);
        return;
    });
}
...
function append_(v1, v2, $c, $p) {
    return __awaiter(this, void 0, void 0, function* () {
        while (!(yield Vector.is_empty_(v2, $c, [$p[0]]))) {
            {
                yield Vector.push_back_(v1, yield Vector.pop_back_(v2, $c, [$p[0]]), $c, [$p[0]]);
            }
        }
        return;
    });
...
function assert_owner_cap_exists_(owner, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(yield $c.exists_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner)))) {
            throw $.abortCode(yield Error.not_found_($.copy(exports.EOWNER_CAP_NOT_FOUND), $c));
        }
        return;
    });
}
...
function assert_stake_pool_exists_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(yield stake_pool_exists_($.copy(pool_address), $c))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.ESTAKE_POOL_DOES_NOT_EXIST), $c));
        }
        return;
    });
}
...
function configure_allowed_validators_(aptos_framework, accounts, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        aptos_framework_address = yield Signer.address_of_(aptos_framework, $c);
        yield System_addresses.assert_aptos_framework_(aptos_framework, $c);
        if (!(yield $c.exists_async(new move_to_ts_2.SimpleStructTag(AllowedValidators), $.copy(aptos_framework_address)))) {
            yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(AllowedValidators), aptos_framework, new AllowedValidators({ accounts: $.copy(accounts) }, new move_to_ts_2.SimpleStructTag(AllowedValidators)));
        }
        else {
            allowed = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(AllowedValidators), $.copy(aptos_framework_address));
...
function deposit_owner_cap_(owner, owner_cap, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!!(yield $c.exists_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), yield Signer.address_of_(owner, $c)))) {
            throw $.abortCode(yield Error.not_found_($.copy(exports.EOWNER_CAP_ALREADY_EXISTS), $c));
        }
        yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), owner, owner_cap);
        return;
    });
}
...
function distribute_rewards_(stake, num_successful_proposals, num_total_proposals, rewards_rate, rewards_rate_denominator, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        stake_amount = yield Coin.value_(stake, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        if (($.copy(stake_amount)).gt((0, move_to_ts_1.u64)("0"))) {
            temp$1 = yield calculate_rewards_amount_($.copy(stake_amount), $.copy(num_successful_proposals), $.copy(num_total_proposals), $.copy(rewards_rate), $.copy(rewards_rate_denominator), $c);
        }
        else {
            temp$1 = (0, move_to_ts_1.u64)("0");
        }
        ...
        if (($.copy(rewards_amount)).gt((0, move_to_ts_1.u64)("0"))) {
            mint_cap = (yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(AptosCoinCapabilities), new aptos_1.HexString("0x1"))).mint_cap;
            rewards = yield Coin.mint_($.copy(rewards_amount), mint_cap, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
            yield Coin.merge_(stake, rewards, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        }
        else {
        }
...
function extract_owner_cap_(owner, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        owner_address = yield Signer.address_of_(owner, $c);
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        return yield $c.move_from_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
    });
}
...
function find_validator_(v, addr, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        len = yield Vector.length_(v, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        while (true) {
            {
                ;
            }
            if (!(($.copy(i)).lt($.copy(len))))
                break;
            {
                if ((($.copy((yield Vector.borrow_(v, $.copy(i), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)])).addr)).hex() === ($.copy(addr)).hex())) {
                    return yield Option.some_($.copy(i), $c, [move_to_ts_2.AtomicTypeTag.U64]);
                }
                else {
                }
        ...
        return yield Option.none_($c, [move_to_ts_2.AtomicTypeTag.U64]);
    });
}
...
function get_current_epoch_proposal_counts_(validator_index, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        validator_performances = (yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(ValidatorPerformance), new aptos_1.HexString("0x1"))).validators;
        validator_performance = yield Vector.borrow_(validator_performances, $.copy(validator_index), $c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]);
...
function get_current_epoch_voting_power_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        validator_state = yield get_validator_state_($.copy(pool_address), $c);
        ...
        if (temp$1) {
            active_stake = yield Coin.value_((yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address))).active, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
            pending_inactive_stake = yield Coin.value_((yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address))).pending_inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
...
function get_delegated_voter_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        return $.copy((yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address))).delegated_voter);
    });
}
...
function get_lockup_secs_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        return $.copy((yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address))).locked_until_secs);
    });
}
...
function get_next_epoch_voting_power_(stake_pool, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        value_pending_active = yield Coin.value_((stake_pool).pending_active, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        value_active = yield Coin.value_((stake_pool).active, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        value_pending_inactive = yield Coin.value_((stake_pool).pending_inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        ;
...
function get_operator_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        return $.copy((yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address))).operator_address);
    });
}
...
function get_remaining_lockup_secs_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        lockup_time = $.copy((yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address))).locked_until_secs);
        if (($.copy(lockup_time)).le(yield Timestamp.now_seconds_($c))) {
            temp$1 = (0, move_to_ts_1.u64)("0");
        }
        else {
            temp$1 = ($.copy(lockup_time)).sub(yield Timestamp.now_seconds_($c));
        }
        return temp$1;
    });
}
...
function get_stake_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        let stake_pool;
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        stake_pool = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        return [yield Coin.value_((stake_pool).active, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), yield Coin.value_((stake_pool).inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), yield Coin.value_((stake_pool).pending_active, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), yield Coin.value_((stake_pool).pending_inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])])];
    });
}
...
function get_validator_config_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        validator_config = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy(pool_address));
...
function get_validator_index_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        return $.copy((yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy(pool_address))).validator_index);
    });
}
...
function get_validator_state_(pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        validator_set = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), new aptos_1.HexString("0x1"));
        temp$1 = yield find_validator_((validator_set).pending_active, $.copy(pool_address), $c);
        if (yield Option.is_some_(temp$1, $c, [move_to_ts_2.AtomicTypeTag.U64])) {
            temp$6 = $.copy(exports.VALIDATOR_STATUS_PENDING_ACTIVE);
        }
        else {
            temp$2 = yield find_validator_((validator_set).active_validators, $.copy(pool_address), $c);
            if (yield Option.is_some_(temp$2, $c, [move_to_ts_2.AtomicTypeTag.U64])) {
                temp$5 = $.copy(exports.VALIDATOR_STATUS_ACTIVE);
            }
            else {
                temp$3 = yield find_validator_((validator_set).pending_inactive, $.copy(pool_address), $c);
                if (yield Option.is_some_(temp$3, $c, [move_to_ts_2.AtomicTypeTag.U64])) {
...
function increase_lockup_(owner, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        ownership_cap = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
        yield increase_lockup_with_cap_(ownership_cap, $c);
        return;
    });
}
...
function increase_lockup_with_cap_(owner_cap, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        pool_address = $.copy((owner_cap).pool_address);
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        config = yield Staking_config.get_($c);
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        old_locked_until_secs = $.copy((stake_pool).locked_until_secs);
        (stake_pool).locked_until_secs = (yield Timestamp.now_seconds_($c)).add(yield Staking_config.get_recurring_lockup_duration_(config, $c));
        yield Event.emit_event_((stake_pool).increase_lockup_events, new IncreaseLockupEvent({ pool_address: $.copy(pool_address), old_locked_until_secs: $.copy(old_locked_until_secs), new_locked_until_secs: $.copy((stake_pool).locked_until_secs) }, new move_to_ts_2.SimpleStructTag(IncreaseLockupEvent)), $c, [new move_to_ts_2.SimpleStructTag(IncreaseLockupEvent)]);
        return;
    });
}
...
function initialize_(aptos_framework, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        temp$2 = yield Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        temp$3 = yield Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        temp$4 = yield Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        temp$5 = (0, move_to_ts_1.u128)("0");
        temp$6 = (0, move_to_ts_1.u128)("0");
        yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), temp$7, new ValidatorSet({ consensus_scheme: temp$1, active_validators: temp$2, pending_inactive: temp$4, pending_active: temp$3, total_voting_power: temp$5, total_joining_power: temp$6 }, new move_to_ts_2.SimpleStructTag(ValidatorSet)));
        yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(ValidatorPerformance), aptos_framework, new ValidatorPerformance({ validators: yield Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]) }, new move_to_ts_2.SimpleStructTag(ValidatorPerformance)));
        return;
    });
}
...
function initialize_owner_(owner, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        owner_address = yield Signer.address_of_(owner, $c);
        if (!(yield is_allowed_($.copy(owner_address), $c))) {
            throw $.abortCode(yield Error.not_found_($.copy(exports.EINELIGIBLE_VALIDATOR), $c));
        }
        if (!!(yield stake_pool_exists_($.copy(owner_address), $c))) {
            throw $.abortCode(yield Error.already_exists_($.copy(exports.EALREADY_REGISTERED), $c));
        }
        temp$20 = owner;
        temp$1 = yield Coin.zero_($c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        temp$2 = yield Coin.zero_($c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        temp$3 = yield Coin.zero_($c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        temp$4 = yield Coin.zero_($c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        temp$5 = (0, move_to_ts_1.u64)("0");
        temp$6 = $.copy(owner_address);
        temp$7 = $.copy(owner_address);
        temp$8 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(RegisterValidatorCandidateEvent)]);
        temp$9 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(SetOperatorEvent)]);
        temp$10 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(AddStakeEvent)]);
        temp$11 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(ReactivateStakeEvent)]);
        temp$12 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(RotateConsensusKeyEvent)]);
        temp$13 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(UpdateNetworkAndFullnodeAddressesEvent)]);
        temp$14 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(IncreaseLockupEvent)]);
        temp$15 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(JoinValidatorSetEvent)]);
        temp$16 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(DistributeRewardsEvent)]);
        temp$17 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(UnlockStakeEvent)]);
        temp$18 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(WithdrawStakeEvent)]);
        temp$19 = yield Account.new_event_handle_(owner, $c, [new move_to_ts_2.SimpleStructTag(LeaveValidatorSetEvent)]);
        yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(StakePool), temp$20, new StakePool({ active: temp$1, inactive: temp$4, pending_active: temp$2, pending_inactive: temp$3, locked_until_secs: temp$5, operator_address: temp$6, delegated_voter: temp$7, initialize_validator_events: temp$8, set_operator_events: temp$9, add_stake_events: temp$10, reactivate_stake_events: temp$11, rotate_consensus_key_events: temp$12, update_network_and_fullnode_addresses_events: temp$13, increase_lockup_events: temp$14, join_validator_set_events: temp$15, distribute_rewards_events: temp$16, unlock_stake_events: temp$17, withdraw_stake_events: temp$18, leave_validator_set_events: temp$19 }, new move_to_ts_2.SimpleStructTag(StakePool)));
        yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), owner, new OwnerCapability({ pool_address: $.copy(owner_address) }, new move_to_ts_2.SimpleStructTag(OwnerCapability)));
        return;
    });
}
...
function initialize_stake_owner_(owner, initial_stake_amount, operator, voter, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        let account_address;
        yield initialize_owner_(owner, $c);
        yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), owner, new ValidatorConfig({ consensus_pubkey: yield Vector.empty_($c, [move_to_ts_2.AtomicTypeTag.U8]), network_addresses: yield Vector.empty_($c, [move_to_ts_2.AtomicTypeTag.U8]), fullnode_addresses: yield Vector.empty_($c, [move_to_ts_2.AtomicTypeTag.U8]), validator_index: (0, move_to_ts_1.u64)("0") }, new move_to_ts_2.SimpleStructTag(ValidatorConfig)));
        if (($.copy(initial_stake_amount)).gt((0, move_to_ts_1.u64)("0"))) {
            yield add_stake_(owner, $.copy(initial_stake_amount), $c);
        }
        else {
        }
        account_address = yield Signer.address_of_(owner, $c);
        if ((($.copy(account_address)).hex() !== ($.copy(operator)).hex())) {
            yield set_operator_(owner, $.copy(operator), $c);
        }
        else {
        }
        if ((($.copy(account_address)).hex() !== ($.copy(voter)).hex())) {
            yield set_delegated_voter_(owner, $.copy(voter), $c);
        }
        else {
        }
        return;
...
function initialize_validator_(account, consensus_pubkey, proof_of_possession, network_addresses, fullnode_addresses, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        temp$1 = yield Bls12381.proof_of_possession_from_bytes_($.copy(proof_of_possession), $c);
        temp$3 = yield Bls12381.public_key_from_bytes_with_pop_(temp$2, temp$1, $c);
        pubkey_from_pop = temp$3;
        if (!(yield Option.is_some_(pubkey_from_pop, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "bls12381", "PublicKeyWithPoP", [])]))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.EINVALID_PUBLIC_KEY), $c));
        }
        yield initialize_owner_(account, $c);
        yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), account, new ValidatorConfig({ consensus_pubkey: $.copy(consensus_pubkey), network_addresses: $.copy(network_addresses), fullnode_addresses: $.copy(fullnode_addresses), validator_index: (0, move_to_ts_1.u64)("0") }, new move_to_ts_2.SimpleStructTag(ValidatorConfig)));
        return;
    });
}
...
function is_allowed_(account, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        let temp$1, allowed;
        if (!(yield $c.exists_async(new move_to_ts_2.SimpleStructTag(AllowedValidators), new aptos_1.HexString("0x1")))) {
            temp$1 = true;
        }
        else {
            allowed = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(AllowedValidators), new aptos_1.HexString("0x1"));
            temp$1 = yield Vector.contains_((allowed).accounts, account, $c, [move_to_ts_2.AtomicTypeTag.Address]);
        }
        return temp$1;
    });
}
...
function join_validator_set_(operator, pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        let temp$1;
        temp$1 = yield Staking_config.get_($c);
        if (!(yield Staking_config.get_allow_validator_set_change_(temp$1, $c))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.ENO_POST_GENESIS_VALIDATOR_SET_CHANGE_ALLOWED), $c));
        }
        yield join_validator_set_internal_(operator, $.copy(pool_address), $c);
        return;
    });
}
...
function join_validator_set_internal_(operator, pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        if (!((yield Signer.address_of_(operator, $c)).hex() === ($.copy((stake_pool).operator_address)).hex())) {
            throw $.abortCode(yield Error.unauthenticated_($.copy(exports.ENOT_OPERATOR), $c));
        }
        if (!(yield get_validator_state_($.copy(pool_address), $c)).eq(($.copy(exports.VALIDATOR_STATUS_INACTIVE)))) {
            throw $.abortCode(yield Error.invalid_state_($.copy(exports.EALREADY_ACTIVE_VALIDATOR), $c));
        }
        config = yield Staking_config.get_($c);
        [minimum_stake, maximum_stake] = yield Staking_config.get_required_stake_(config, $c);
        voting_power = yield get_next_epoch_voting_power_(stake_pool, $c);
        if (!($.copy(voting_power)).ge($.copy(minimum_stake))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.ESTAKE_TOO_LOW), $c));
        }
        if (!($.copy(voting_power)).le($.copy(maximum_stake))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.ESTAKE_TOO_HIGH), $c));
        }
        yield update_voting_power_increase_($.copy(voting_power), $c);
        validator_config = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy(pool_address));
        if (!!(yield Vector.is_empty_((validator_config).consensus_pubkey, $c, [move_to_ts_2.AtomicTypeTag.U8]))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.EINVALID_PUBLIC_KEY), $c));
        }
        validator_set = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), new aptos_1.HexString("0x1"));
        temp$4 = (validator_set).pending_active;
        [temp$1, temp$2, temp$3] = [$.copy(pool_address), stake_pool, $.copy(validator_config)];
        yield Vector.push_back_(temp$4, yield generate_validator_info_(temp$1, temp$2, temp$3, $c), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        validator_set_size = (yield Vector.length_((validator_set).active_validators, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)])).add(yield Vector.length_((validator_set).pending_active, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]));
        if (!($.copy(validator_set_size)).le($.copy(exports.MAX_VALIDATOR_SET_SIZE))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.EVALIDATOR_SET_TOO_LARGE), $c));
        }
        yield Event.emit_event_((stake_pool).join_validator_set_events, new JoinValidatorSetEvent({ pool_address: $.copy(pool_address) }, new move_to_ts_2.SimpleStructTag(JoinValidatorSetEvent)), $c, [new move_to_ts_2.SimpleStructTag(JoinValidatorSetEvent)]);
        return;
    });
}
...
function leave_validator_set_(operator, pool_address, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        config = yield Staking_config.get_($c);
        if (!(yield Staking_config.get_allow_validator_set_change_(config, $c))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.ENO_POST_GENESIS_VALIDATOR_SET_CHANGE_ALLOWED), $c));
        }
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        ...
        validator_set = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), new aptos_1.HexString("0x1"));
        maybe_pending_active_index = yield find_validator_((validator_set).pending_active, $.copy(pool_address), $c);
        if (yield Option.is_some_(maybe_pending_active_index, $c, [move_to_ts_2.AtomicTypeTag.U64])) {
            yield Vector.swap_remove_((validator_set).pending_active, yield Option.extract_(maybe_pending_active_index, $c, [move_to_ts_2.AtomicTypeTag.U64]), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
            validator_stake = (0, move_to_ts_1.u128)(yield get_next_epoch_voting_power_(stake_pool, $c));
            ...
            maybe_active_index = yield find_validator_((validator_set).active_validators, $.copy(pool_address), $c);
            if (!(yield Option.is_some_(maybe_active_index, $c, [move_to_ts_2.AtomicTypeTag.U64]))) {
                throw $.abortCode(yield Error.invalid_state_($.copy(exports.ENOT_VALIDATOR), $c));
            }
            validator_info = yield Vector.swap_remove_((validator_set).active_validators, yield Option.extract_(maybe_active_index, $c, [move_to_ts_2.AtomicTypeTag.U64]), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
            if (!(yield Vector.length_((validator_set).active_validators, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)])).gt((0, move_to_ts_1.u64)("0"))) {
                throw $.abortCode(yield Error.invalid_state_($.copy(exports.ELAST_VALIDATOR), $c));
            }
            yield Vector.push_back_((validator_set).pending_inactive, $.copy(validator_info), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
            yield Event.emit_event_((stake_pool).leave_validator_set_events, new LeaveValidatorSetEvent({ pool_address: $.copy(pool_address) }, new move_to_ts_2.SimpleStructTag(LeaveValidatorSetEvent)), $c, [new move_to_ts_2.SimpleStructTag(LeaveValidatorSetEvent)]);
        }
        return;
    });
}
...
function on_new_epoch_($c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        validator_set = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), new aptos_1.HexString("0x1"));
        config = yield Staking_config.get_($c);
        validator_perf = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorPerformance), new aptos_1.HexString("0x1"));
        i = (0, move_to_ts_1.u64)("0");
        len = yield Vector.length_((validator_set).active_validators, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        while (($.copy(i)).lt($.copy(len))) {
            {
                validator = yield Vector.borrow_((validator_set).active_validators, $.copy(i), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
                [temp$1, temp$2, temp$3] = [validator_perf, $.copy((validator).addr), config];
                yield update_stake_pool_(temp$1, temp$2, temp$3, $c);
                i = ($.copy(i)).add((0, move_to_ts_1.u64)("1"));
            }
        }
        i__4 = (0, move_to_ts_1.u64)("0");
        len__5 = yield Vector.length_((validator_set).pending_inactive, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        while (($.copy(i__4)).lt($.copy(len__5))) {
            {
                validator__6 = yield Vector.borrow_((validator_set).pending_inactive, $.copy(i__4), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
                [temp$7, temp$8, temp$9] = [validator_perf, $.copy((validator__6).addr), config];
                yield update_stake_pool_(temp$7, temp$8, temp$9, $c);
                i__4 = ($.copy(i__4)).add((0, move_to_ts_1.u64)("1"));
            }
        }
        yield append_((validator_set).active_validators, (validator_set).pending_active, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        (validator_set).pending_inactive = yield Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        next_epoch_validators = yield Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        [minimum_stake,] = yield Staking_config.get_required_stake_(config, $c);
        vlen = yield Vector.length_((validator_set).active_validators, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        ...
        while (true) {
            ...
            if (!(($.copy(i__10)).lt($.copy(vlen))))
                break;
            {
                old_validator_info = yield Vector.borrow_mut_((validator_set).active_validators, $.copy(i__10), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
                pool_address = $.copy((old_validator_info).addr);
                validator_config = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy(pool_address));
                stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
                [temp$11, temp$12, temp$13] = [$.copy(pool_address), stake_pool, $.copy(validator_config)];
                new_validator_info = yield generate_validator_info_(temp$11, temp$12, temp$13, $c);
                if (($.copy((new_validator_info).voting_power)).ge($.copy(minimum_stake))) {
                    ...
                    yield Vector.push_back_(next_epoch_validators, $.copy(new_validator_info), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
                }
                else {
                }
        ...
        (validator_set).total_joining_power = (0, move_to_ts_1.u128)("0");
        (validator_perf).validators = yield Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]);
        recurring_lockup_duration_secs = yield Staking_config.get_recurring_lockup_duration_(config, $c);
        vlen__14 = yield Vector.length_((validator_set).active_validators, $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
        ...
        while (true) {
            ...
            if (!(($.copy(validator_index)).lt($.copy(vlen__14))))
                break;
            {
                validator_info = yield Vector.borrow_mut_((validator_set).active_validators, $.copy(validator_index), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
                ((validator_info).config).validator_index = $.copy(validator_index);
                validator_config__15 = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy((validator_info).addr));
                (validator_config__15).validator_index = $.copy(validator_index);
                yield Vector.push_back_((validator_perf).validators, new IndividualValidatorPerformance({ successful_proposals: (0, move_to_ts_1.u64)("0"), failed_proposals: (0, move_to_ts_1.u64)("0") }, new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)), $c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]);
                stake_pool__16 = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy((validator_info).addr));
                if (($.copy((stake_pool__16).locked_until_secs)).le(yield Timestamp.now_seconds_($c))) {
                    ;
                    (stake_pool__16).locked_until_secs = (yield Timestamp.now_seconds_($c)).add($.copy(recurring_lockup_duration_secs));
                }
                else {
                }
...
function reactivate_stake_(owner, amount, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        ownership_cap = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
        yield reactivate_stake_with_cap_(ownership_cap, $.copy(amount), $c);
        return;
    });
}
...
function reactivate_stake_with_cap_(owner_cap, amount, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        total_pending_inactive = yield Coin.value_((stake_pool).pending_inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        amount = yield Math64.min_($.copy(amount), $.copy(total_pending_inactive), $c);
        reactivated_coins = yield Coin.extract_((stake_pool).pending_inactive, $.copy(amount), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        yield Coin.merge_((stake_pool).active, reactivated_coins, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        yield Event.emit_event_((stake_pool).reactivate_stake_events, new ReactivateStakeEvent({ pool_address: $.copy(pool_address), amount: $.copy(amount) }, new move_to_ts_2.SimpleStructTag(ReactivateStakeEvent)), $c, [new move_to_ts_2.SimpleStructTag(ReactivateStakeEvent)]);
        return;
    });
}
...
function remove_validators_(aptos_framework, validators, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield System_addresses.assert_aptos_framework_(aptos_framework, $c);
        validator_set = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), new aptos_1.HexString("0x1"));
        yield remove_validators_internal_((validator_set).active_validators, validators, $c);
        return;
    });
}
exports.remove_validators_ = remove_validators_;
function remove_validators_internal_(active_validators, validators, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        let temp$1, temp$2, i, len, validator, validator_index;
        len = yield Vector.length_(validators, $c, [move_to_ts_2.AtomicTypeTag.Address]);
        i = (0, move_to_ts_1.u64)("0");
        while (true) {
            {
                ;
            ...
            if (!(($.copy(i)).lt($.copy(len))))
                break;
            {
                validator = $.copy(yield Vector.borrow_(validators, $.copy(i), $c, [move_to_ts_2.AtomicTypeTag.Address]));
                [temp$1, temp$2] = [active_validators, $.copy(validator)];
                validator_index = yield find_validator_(temp$1, temp$2, $c);
                if (yield Option.is_some_(validator_index, $c, [move_to_ts_2.AtomicTypeTag.U64])) {
                    yield Vector.remove_(active_validators, $.copy(yield Option.borrow_(validator_index, $c, [move_to_ts_2.AtomicTypeTag.U64])), $c, [new move_to_ts_2.SimpleStructTag(ValidatorInfo)]);
                }
                else {
                }
                i = ($.copy(i)).add((0, move_to_ts_1.u64)("1"));
...
exports.remove_validators_internal_ = remove_validators_internal_;
function rotate_consensus_key_(operator, pool_address, new_consensus_pubkey, proof_of_possession, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!(yield $c.exists_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy(pool_address)))) {
            throw $.abortCode(yield Error.not_found_($.copy(exports.EVALIDATOR_CONFIG), $c));
        }
        validator_info = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy(pool_address));
...
function set_delegated_voter_(owner, new_voter, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        ownership_cap = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
        yield set_delegated_voter_with_cap_(ownership_cap, $.copy(new_voter), $c);
        return;
    });
}
...
function set_operator_(owner, new_operator, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        ownership_cap = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
        yield set_operator_with_cap_(ownership_cap, $.copy(new_operator), $c);
        return;
    });
}
...
function set_operator_with_cap_(owner_cap, new_operator, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield Event.emit_event_((stake_pool).set_operator_events, new SetOperatorEvent({ pool_address: $.copy(pool_address), old_operator: $.copy(old_operator), new_operator: $.copy(new_operator) }, new move_to_ts_2.SimpleStructTag(SetOperatorEvent)), $c, [new move_to_ts_2.SimpleStructTag(SetOperatorEvent)]);
        return;
    });
}
...
function stake_pool_exists_(addr, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield $c.exists_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(addr));
    });
}
...
function store_aptos_coin_mint_cap_(aptos_framework, mint_cap, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        yield System_addresses.assert_aptos_framework_(aptos_framework, $c);
        return yield $c.move_to_async(new move_to_ts_2.SimpleStructTag(AptosCoinCapabilities), aptos_framework, new AptosCoinCapabilities({ mint_cap: $.copy(mint_cap) }, new move_to_ts_2.SimpleStructTag(AptosCoinCapabilities)));
    });
}
...
function unlock_(owner, amount, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        ownership_cap = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
        yield unlock_with_cap_($.copy(amount), ownership_cap, $c);
        return;
    });
}
...
function unlock_with_cap_(amount, owner_cap, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_stake_pool_exists_($.copy(pool_address), $c);
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        amount__1 = yield Math64.min_($.copy(amount), yield Coin.value_((stake_pool).active, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), $c);
        unlocked_stake = yield Coin.extract_((stake_pool).active, $.copy(amount__1), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        yield Coin.merge_((stake_pool).pending_inactive, unlocked_stake, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        yield Event.emit_event_((stake_pool).unlock_stake_events, new UnlockStakeEvent({ pool_address: $.copy(pool_address), amount_unlocked: $.copy(amount__1) }, new move_to_ts_2.SimpleStructTag(UnlockStakeEvent)), $c, [new move_to_ts_2.SimpleStructTag(UnlockStakeEvent)]);
        return;
    });
}
...
function update_performance_statistics_(proposer_index, failed_proposer_indices, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        validator_perf = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorPerformance), new aptos_1.HexString("0x1"));
        validator_len = yield Vector.length_((validator_perf).validators, $c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]);
        if (yield Option.is_some_(proposer_index, $c, [move_to_ts_2.AtomicTypeTag.U64])) {
            cur_proposer_index = yield Option.extract_(proposer_index, $c, [move_to_ts_2.AtomicTypeTag.U64]);
            if (($.copy(cur_proposer_index)).lt($.copy(validator_len))) {
                validator = yield Vector.borrow_mut_((validator_perf).validators, $.copy(cur_proposer_index), $c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]);
                ;
        ...
        f_len = yield Vector.length_(failed_proposer_indices, $c, [move_to_ts_2.AtomicTypeTag.U64]);
        while (true) {
            {
                ;
            }
            if (!(($.copy(f)).lt($.copy(f_len))))
                break;
            {
                validator_index = $.copy(yield Vector.borrow_(failed_proposer_indices, $.copy(f), $c, [move_to_ts_2.AtomicTypeTag.U64]));
                if (($.copy(validator_index)).lt($.copy(validator_len))) {
                    validator__1 = yield Vector.borrow_mut_((validator_perf).validators, $.copy(validator_index), $c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]);
                    ;
...
function update_stake_pool_(validator_perf, pool_address, staking_config, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        validator_config = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(ValidatorConfig), $.copy(pool_address));
        cur_validator_perf = yield Vector.borrow_((validator_perf).validators, $.copy((validator_config).validator_index), $c, [new move_to_ts_2.SimpleStructTag(IndividualValidatorPerformance)]);
        ...
        [rewards_rate, rewards_rate_denominator] = yield Staking_config.get_reward_rate_(staking_config, $c);
        rewards_active = yield distribute_rewards_((stake_pool).active, $.copy(num_successful_proposals), $.copy(num_total_proposals), $.copy(rewards_rate), $.copy(rewards_rate_denominator), $c);
        rewards_pending_inactive = yield distribute_rewards_((stake_pool).pending_inactive, $.copy(num_successful_proposals), $.copy(num_total_proposals), $.copy(rewards_rate), $.copy(rewards_rate_denominator), $c);
        ;
        rewards_amount = ($.copy(rewards_active)).add($.copy(rewards_pending_inactive));
        yield Coin.merge_((stake_pool).active, yield Coin.extract_all_((stake_pool).pending_active, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        current_lockup_expiration = $.copy((stake_pool).locked_until_secs);
        if ((yield Timestamp.now_seconds_($c)).ge($.copy(current_lockup_expiration))) {
            yield Coin.merge_((stake_pool).inactive, yield Coin.extract_all_((stake_pool).pending_inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        }
        else {
        }
        yield Event.emit_event_((stake_pool).distribute_rewards_events, new DistributeRewardsEvent({ pool_address: $.copy(pool_address), rewards_amount: $.copy(rewards_amount) }, new move_to_ts_2.SimpleStructTag(DistributeRewardsEvent)), $c, [new move_to_ts_2.SimpleStructTag(DistributeRewardsEvent)]);
        return;
    });
}
...
function update_voting_power_increase_(increase_amount, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        validator_set = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(ValidatorSet), new aptos_1.HexString("0x1"));
        temp$1 = yield Staking_config.get_($c);
        voting_power_increase_limit = (0, move_to_ts_1.u128)(yield Staking_config.get_voting_power_increase_limit_(temp$1, $c));
        ...
        if (($.copy((validator_set).total_voting_power)).gt((0, move_to_ts_1.u128)("0"))) {
            if (!($.copy((validator_set).total_joining_power)).le((($.copy((validator_set).total_voting_power)).mul($.copy(voting_power_increase_limit))).div((0, move_to_ts_1.u128)("100")))) {
                throw $.abortCode(yield Error.invalid_argument_($.copy(exports.EVOTING_POWER_INCREASE_EXCEEDS_LIMIT), $c));
            }
        }
        else {
        }
...
function withdraw_(owner, withdraw_amount, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield assert_owner_cap_exists_($.copy(owner_address), $c);
        ownership_cap = yield $c.borrow_global_async(new move_to_ts_2.SimpleStructTag(OwnerCapability), $.copy(owner_address));
        coins = yield withdraw_with_cap_(ownership_cap, $.copy(withdraw_amount), $c);
        yield Coin.deposit_($.copy(owner_address), coins, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        return;
    });
}
...
function withdraw_with_cap_(owner_cap, withdraw_amount, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        stake_pool = yield $c.borrow_global_mut_async(new move_to_ts_2.SimpleStructTag(StakePool), $.copy(pool_address));
        if ((yield get_validator_state_($.copy(pool_address), $c)).eq(($.copy(exports.VALIDATOR_STATUS_INACTIVE)))) {
            temp$1 = (yield Timestamp.now_seconds_($c)).ge($.copy((stake_pool).locked_until_secs));
        }
        else {
            temp$1 = false;
        }
        if (temp$1) {
            pending_inactive_stake = yield Coin.extract_all_((stake_pool).pending_inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
            yield Coin.merge_((stake_pool).inactive, pending_inactive_stake, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        }
        else {
        }
        withdraw_amount = yield Math64.min_($.copy(withdraw_amount), yield Coin.value_((stake_pool).inactive, $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]), $c);
        if (($.copy(withdraw_amount)).eq(((0, move_to_ts_1.u64)("0")))) {
            return yield Coin.zero_($c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
        }
        else {
        }
        yield Event.emit_event_((stake_pool).withdraw_stake_events, new WithdrawStakeEvent({ pool_address: $.copy(pool_address), amount_withdrawn: $.copy(withdraw_amount) }, new move_to_ts_2.SimpleStructTag(WithdrawStakeEvent)), $c, [new move_to_ts_2.SimpleStructTag(WithdrawStakeEvent)]);
        return yield Coin.extract_((stake_pool).inactive, $.copy(withdraw_amount), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
    });
}
