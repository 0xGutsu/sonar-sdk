...
function ecdsa_raw_public_key_from_64_bytes_(bytes, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(yield Vector.length_(bytes, $c, [move_to_ts_2.AtomicTypeTag.U8])).eq(($.copy(exports.RAW_PUBLIC_KEY_NUM_BYTES)))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.E_DESERIALIZE), $c));
        }
...
function ecdsa_recover_(message, recovery_id, signature, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        [pk, success] = yield ecdsa_recover_internal_($.copy(message), $.copy(recovery_id), $.copy((signature).bytes), $c);
        if (success) {
            temp$1 = yield Option.some_(yield ecdsa_raw_public_key_from_64_bytes_($.copy(pk), $c), $c, [new move_to_ts_2.SimpleStructTag(ECDSARawPublicKey)]);
        }
        else {
            temp$1 = yield Option.none_($c, [new move_to_ts_2.SimpleStructTag(ECDSARawPublicKey)]);
        }
        return temp$1;
    });
}
