...
function epilogue_(account, _txn_sequence_number, txn_gas_price, txn_max_gas_units, gas_units_remaining, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!($.copy(txn_max_gas_units)).ge($.copy(gas_units_remaining))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.EOUT_OF_GAS), $c));
        }
        ...
        if (!(((0, move_to_ts_1.u128)($.copy(txn_gas_price))).mul((0, move_to_ts_1.u128)($.copy(gas_used)))).le($.copy(exports.MAX_U64))) {
            throw $.abortCode(yield Error.out_of_range_($.copy(exports.EOUT_OF_GAS), $c));
        }
        transaction_fee_amount = ($.copy(txn_gas_price)).mul($.copy(gas_used));
        addr = yield Signer.address_of_(account, $c);
        if (!(yield Coin.balance_($.copy(addr), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])])).ge($.copy(transaction_fee_amount))) {
            throw $.abortCode(yield Error.out_of_range_($.copy(exports.PROLOGUE_ECANT_PAY_GAS_DEPOSIT), $c));
        }
        yield Transaction_fee.burn_fee_($.copy(addr), $.copy(transaction_fee_amount), $c);
        yield Account.increment_sequence_number_($.copy(addr), $c);
        return;
    });
}
...
function module_prologue_(sender, txn_sequence_number, txn_public_key, txn_gas_price, txn_max_gas_units, txn_expiration_time, chain_id, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield prologue_common_(sender, $.copy(txn_sequence_number), $.copy(txn_public_key), $.copy(txn_gas_price), $.copy(txn_max_gas_units), $.copy(txn_expiration_time), $.copy(chain_id), $c);
    });
}
...
function multi_agent_script_prologue_(sender, txn_sequence_number, txn_sender_public_key, secondary_signer_addresses, secondary_signer_public_key_hashes, txn_gas_price, txn_max_gas_units, txn_expiration_time, chain_id, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield prologue_common_(sender, $.copy(txn_sequence_number), $.copy(txn_sender_public_key), $.copy(txn_gas_price), $.copy(txn_max_gas_units), $.copy(txn_expiration_time), $.copy(chain_id), $c);
        num_secondary_signers = yield Vector.length_(secondary_signer_addresses, $c, [move_to_ts_2.AtomicTypeTag.Address]);
        if (!(yield Vector.length_(secondary_signer_public_key_hashes, $c, [new move_to_ts_2.VectorTag(move_to_ts_2.AtomicTypeTag.U8)])).eq(($.copy(num_secondary_signers)))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_ESECONDARY_KEYS_ADDRESSES_COUNT_MISMATCH), $c));
        }
        i = (0, move_to_ts_1.u64)("0");
        while (($.copy(i)).lt($.copy(num_secondary_signers))) {
            {
                secondary_address = $.copy(yield Vector.borrow_(secondary_signer_addresses, $.copy(i), $c, [move_to_ts_2.AtomicTypeTag.Address]));
                if (!(yield Account.exists_at_($.copy(secondary_address), $c))) {
                    throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_EACCOUNT_DOES_NOT_EXIST), $c));
                }
                signer_public_key_hash = $.copy(yield Vector.borrow_(secondary_signer_public_key_hashes, $.copy(i), $c, [new move_to_ts_2.VectorTag(move_to_ts_2.AtomicTypeTag.U8)]));
                if (!$.veq($.copy(signer_public_key_hash), yield Account.get_authentication_key_($.copy(secondary_address), $c))) {
                    throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_EINVALID_ACCOUNT_AUTH_KEY), $c));
                }
...
function prologue_common_(sender, txn_sequence_number, txn_authentication_key, txn_gas_price, txn_max_gas_units, txn_expiration_time, chain_id, $c) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!(yield Timestamp.now_seconds_($c)).lt($.copy(txn_expiration_time))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_ETRANSACTION_EXPIRED), $c));
        }
        if (!(yield Chain_id.get_($c)).eq(($.copy(chain_id)))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_EBAD_CHAIN_ID), $c));
        }
        transaction_sender = yield Signer.address_of_(sender, $c);
        if (!(yield Account.exists_at_($.copy(transaction_sender), $c))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_EACCOUNT_DOES_NOT_EXIST), $c));
        }
        if (!$.veq($.copy(txn_authentication_key), yield Account.get_authentication_key_($.copy(transaction_sender), $c))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_EINVALID_ACCOUNT_AUTH_KEY), $c));
        }
        if (!((0, move_to_ts_1.u128)($.copy(txn_sequence_number))).lt($.copy(exports.MAX_U64))) {
            throw $.abortCode(yield Error.out_of_range_($.copy(exports.PROLOGUE_ESEQUENCE_NUMBER_TOO_BIG), $c));
        }
        account_sequence_number = yield Account.get_sequence_number_($.copy(transaction_sender), $c);
        if (!($.copy(txn_sequence_number)).ge($.copy(account_sequence_number))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_ESEQUENCE_NUMBER_TOO_OLD), $c));
        }
        if (!($.copy(txn_sequence_number)).eq(($.copy(account_sequence_number)))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_ESEQUENCE_NUMBER_TOO_NEW), $c));
        }
        max_transaction_fee = ($.copy(txn_gas_price)).mul($.copy(txn_max_gas_units));
        if (!(yield Coin.is_account_registered_($.copy(transaction_sender), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]))) {
            throw $.abortCode(yield Error.invalid_argument_($.copy(exports.PROLOGUE_ECANT_PAY_GAS_DEPOSIT), $c));
        }
        balance = yield Coin.balance_($.copy(transaction_sender), $c, [new move_to_ts_2.StructTag(new aptos_1.HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
