...
CritBitTree.fields = [
    ...
    { name: "inner_nodes", typeTag: new move_to_ts_2.VectorTag(new move_to_ts_2.StructTag(new aptos_1.HexString("0x91f6206a9ee4f969adf1e987b11a6a7ff4848fdb63b90f05ecd8a1a73b0e3efa"), "critbit", "InnerNode", [])) },
    { name: "outer_nodes", typeTag: new move_to_ts_2.VectorTag(new move_to_ts_2.StructTag(new aptos_1.HexString("0x91f6206a9ee4f969adf1e987b11a6a7ff4848fdb63b90f05ecd8a1a73b0e3efa"), "critbit", "OuterNode", [new $.TypeParamIdx(0)])) }
];
class InnerNode {
    constructor(proto, typeTag) {
        this.typeTag = typeTag;
...
function borrow_closest_outer_node_(tree, key, $c, $p) {
    ...
    if (is_outer_node_($.copy(tree.root), $c)) {
        return Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(tree.root), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
    }
    else {
    }
    node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(tree.root), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    while (true) {
        ...
        if (is_outer_node_($.copy(child_index), $c)) {
            return Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(child_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
        }
        else {
        }
        node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    }
}
...
function borrow_closest_outer_node_mut_(tree, key, $c, $p) {
    let temp$1, child_index, node;
    if (is_outer_node_($.copy(tree.root), $c)) {
        return Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(tree.root), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
    }
    else {
    }
    node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(tree.root), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    while (true) {
        ...
        child_index = temp$1;
        if (is_outer_node_($.copy(child_index), $c)) {
            return Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(child_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
        }
        else {
        }
        node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
...
function destroy_empty_(tree, $c, $p) {
    ...
    Stdlib.Vector.destroy_empty_(inner_nodes, $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    Stdlib.Vector.destroy_empty_(outer_nodes, $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
    return;
}
exports.destroy_empty_ = destroy_empty_;
function empty_($c, $p) {
    return new CritBitTree({ root: (0, move_to_ts_1.u64)("0"), inner_nodes: Stdlib.Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(InnerNode)]), outer_nodes: Stdlib.Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]) }, new move_to_ts_2.SimpleStructTag(CritBitTree, [$p[0]]));
}
...
function insert_above_(tree, key, value, n_outer_nodes, n_inner_nodes, search_parent_index, critical_bit, $c, $p) {
    let node, node_index;
    node_index = $.copy(Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(search_parent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).parent_index);
    while (true) {
        if (($.copy(node_index)).eq(($.copy(exports.ROOT)))) {
            ...
            node = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(node_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
...
function insert_above_root_(tree, key, value, n_outer_nodes, n_inner_nodes, critical_bit, $c, $p) {
    ...
    Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(old_root_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).parent_index = $.copy(n_inner_nodes);
    tree.root = $.copy(n_inner_nodes);
    push_back_insert_nodes_(tree, $.copy(key), value, $.copy(n_inner_nodes), $.copy(critical_bit), $.copy(exports.ROOT), is_set_($.copy(key), $.copy(critical_bit), $c), $.copy(old_root_index), outer_node_child_index_($.copy(n_outer_nodes), $c), $c, [$p[0]]);
    return;
}
...
function insert_below_(tree, key, value, n_outer_nodes, n_inner_nodes, search_index, search_child_side, search_key, search_parent_index, critical_bit, $c, $p) {
    let search_parent;
    search_parent = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(search_parent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    ...
    Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(search_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).parent_index = $.copy(n_inner_nodes);
...
function insert_below_walk_(tree, key, value, n_outer_nodes, n_inner_nodes, review_node_index, critical_bit, $c, $p) {
    ...
    review_node = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(review_node_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    ...
    Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(walked_child_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).parent_index = $.copy(n_inner_nodes);
...
function insert_empty_(tree, key, value, $c, $p) {
    Stdlib.Vector.push_back_(tree.outer_nodes, new OuterNode({ key: $.copy(key), value: value, parent_index: $.copy(exports.ROOT) }, new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
...
function insert_general_(tree, key, value, n_outer_nodes, $c, $p) {
    ...
    n_inner_nodes = Stdlib.Vector.length_(tree.inner_nodes, $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
...
function insert_singleton_(tree, key, value, $c, $p) {
    ...
    outer_node = Stdlib.Vector.borrow_(tree.outer_nodes, (0, move_to_ts_1.u64)("0"), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
    if (!($.copy(key)).neq($.copy(outer_node.key))) {
        throw $.abortCode($.copy(exports.E_HAS_KEY));
    }
    critical_bit = crit_bit_($.copy(outer_node.key), $.copy(key), $c);
    push_back_insert_nodes_(tree, $.copy(key), value, (0, move_to_ts_1.u64)("0"), $.copy(critical_bit), $.copy(exports.ROOT), ($.copy(key)).gt($.copy(outer_node.key)), outer_node_child_index_((0, move_to_ts_1.u64)("0"), $c), outer_node_child_index_((0, move_to_ts_1.u64)("1"), $c), $c, [$p[0]]);
    tree.root = (0, move_to_ts_1.u64)("0");
    Stdlib.Vector.borrow_mut_(tree.outer_nodes, (0, move_to_ts_1.u64)("0"), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).parent_index = (0, move_to_ts_1.u64)("0");
    return;
}
exports.insert_singleton_ = insert_singleton_;
function is_empty_(tree, $c, $p) {
    return Stdlib.Vector.is_empty_(tree.outer_nodes, $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
}
...
function length_(tree, $c, $p) {
    return Stdlib.Vector.length_(tree.outer_nodes, $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
}
...
function max_key_(tree, $c, $p) {
    ...
    return $.copy(Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_(max_node_child_index_(tree, $c, [$p[0]]), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).key);
}
...
function max_node_child_index_(tree, $c, $p) {
    ...
    while (true) {
        ...
        child_field_index = $.copy(Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_field_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).right_child_index);
    }
}
...
function min_key_(tree, $c, $p) {
    ...
    return $.copy(Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_(min_node_child_index_(tree, $c, [$p[0]]), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).key);
}
...
function min_node_child_index_(tree, $c, $p) {
    ...
    while (true) {
        ...
        child_field_index = $.copy(Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_field_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).left_child_index);
    }
}
...
function pop_destroy_nodes_(tree, inner_index, outer_index, n_outer_nodes, $c, $p) {
    let n_inner_nodes;
    n_inner_nodes = Stdlib.Vector.length_(tree.inner_nodes, $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    Stdlib.Vector.swap_remove_(tree.inner_nodes, $.copy(inner_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    ...
    let { value: value } = Stdlib.Vector.swap_remove_(tree.outer_nodes, outer_node_vector_index_($.copy(outer_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
    if ((outer_node_vector_index_($.copy(outer_index), $c)).lt(($.copy(n_outer_nodes)).sub((0, move_to_ts_1.u64)("1")))) {
...
function pop_singleton_(tree, key, $c, $p) {
    if (!($.copy(Stdlib.Vector.borrow_(tree.outer_nodes, (0, move_to_ts_1.u64)("0"), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).key)).eq(($.copy(key)))) {
        throw $.abortCode($.copy(exports.E_NOT_HAS_KEY));
    }
    tree.root = (0, move_to_ts_1.u64)("0");
    let { value: value } = Stdlib.Vector.pop_back_(tree.outer_nodes, $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
    return value;
}
...
function pop_update_relationships_(tree, child_side, parent_index, $c, $p) {
    ...
    parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    ...
    if (is_outer_node_($.copy(sibling_index), $c)) {
        Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(sibling_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).parent_index = $.copy(grandparent_index);
    }
    else {
        Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(sibling_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).parent_index = $.copy(grandparent_index);
    }
    if (($.copy(grandparent_index)).eq(($.copy(exports.ROOT)))) {
        tree.root = $.copy(sibling_index);
    }
    else {
        grandparent = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(grandparent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
...
function push_back_insert_nodes_(tree, key, value, inner_index, critical_bit, parent_index, child_polarity, child_index_1, child_index_2, $c, $p) {
    ...
    Stdlib.Vector.push_back_(tree.outer_nodes, new OuterNode({ key: $.copy(key), value: value, parent_index: $.copy(inner_index) }, new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
    Stdlib.Vector.push_back_(tree.inner_nodes, new InnerNode({ critical_bit: $.copy(critical_bit), parent_index: $.copy(parent_index), left_child_index: $.copy(left_child_index), right_child_index: $.copy(right_child_index) }, new move_to_ts_2.SimpleStructTag(InnerNode)), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    return;
}
...
function search_outer_(tree, key, $c, $p) {
    ...
    parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(tree.root), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    while (true) {
        ...
        if (is_outer_node_($.copy(index), $c)) {
            node = Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
            return [$.copy(index), side, $.copy(node.key), $.copy(node.parent_index), $.copy(parent.critical_bit)];
        }
        else {
        }
        parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
    }
}
exports.search_outer_ = search_outer_;
function singleton_(key, value, $c, $p) {
    let tree;
    tree = new CritBitTree({ root: (0, move_to_ts_1.u64)("0"), inner_nodes: Stdlib.Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(InnerNode)]), outer_nodes: Stdlib.Vector.empty_($c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]) }, new move_to_ts_2.SimpleStructTag(CritBitTree, [$p[0]]));
    insert_empty_(tree, $.copy(key), value, $c, [$p[0]]);
    return tree;
}
...
function stitch_child_of_parent_(tree, new_index, parent_index, old_index, $c, $p) {
    let parent;
    parent = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(parent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
...
function stitch_parent_of_child_(tree, new_index, child_index, $c, $p) {
    if (is_outer_node_($.copy(child_index), $c)) {
        Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(child_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).parent_index = $.copy(new_index);
    }
    else {
        Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(child_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).parent_index = $.copy(new_index);
    }
    return;
}
...
function stitch_swap_remove_(tree, node_index, n_nodes, $c, $p) {
    ...
    if (is_outer_node_($.copy(node_index), $c)) {
        parent_index = $.copy(Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(node_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]).parent_index);
        ...
        stitch_child_of_parent_(tree, $.copy(node_index), $.copy(parent_index), outer_node_child_index_(($.copy(n_nodes)).sub((0, move_to_ts_1.u64)("1")), $c), $c, [$p[0]]);
    }
    else {
        node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(node_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
        ...
        stitch_child_of_parent_(tree, $.copy(node_index), $.copy(parent_index__1), ($.copy(n_nodes)).sub((0, move_to_ts_1.u64)("1")), $c, [$p[0]]);
    }
    return;
}
...
function traverse_end_pop_(tree, parent_index, child_index, n_outer_nodes, $c, $p) {
    ...
    if (($.copy(n_outer_nodes)).eq(((0, move_to_ts_1.u64)("1")))) {
        ...
        node_child_side = ($.copy(Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).left_child_index)).eq(($.copy(child_index)));
...
function traverse_init_mut_(tree, direction, $c, $p) {
    ...
    node = Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(child_field_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
...
function traverse_mut_(tree, key, parent_index, direction, $c, $p) {
    ...
    node = Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(target_child_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
...
function traverse_pop_mut_(tree, key, parent_index, child_index, n_outer_nodes, direction, $c, $p) {
    ...
    start_child_side = ($.copy(Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).left_child_index)).eq(($.copy(child_index)));
    [temp$1, temp$2, temp$3, temp$4] = [tree, $.copy(key), $.copy(parent_index), direction];
    target_child_index = traverse_target_child_index_(temp$1, temp$2, temp$3, temp$4, $c, [$p[0]]);
    ...
    if ((outer_node_vector_index_($.copy(target_child_index), $c)).eq((($.copy(n_outer_nodes)).sub((0, move_to_ts_1.u64)("1"))))) {
        target_child_index = $.copy(child_index);
    }
    else {
    }
    target_node = Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(target_child_index), $c), $c, [new move_to_ts_2.SimpleStructTag(OuterNode, [$p[0]])]);
...
function traverse_predecessor_init_mut_(tree, $c, $p) {
    return traverse_init_mut_(tree, $.copy(exports.LEFT), $c, [$p[0]]);
}
...
function traverse_predecessor_mut_(tree, key, parent_index, $c, $p) {
    return traverse_mut_(tree, $.copy(key), $.copy(parent_index), $.copy(exports.LEFT), $c, [$p[0]]);
}
...
function traverse_predecessor_pop_mut_(tree, key, parent_index, child_index, n_outer_nodes, $c, $p) {
    return traverse_pop_mut_(tree, $.copy(key), $.copy(parent_index), $.copy(child_index), $.copy(n_outer_nodes), $.copy(exports.LEFT), $c, [$p[0]]);
}
exports.traverse_predecessor_pop_mut_ = traverse_predecessor_pop_mut_;
function traverse_successor_init_mut_(tree, $c, $p) {
    return traverse_init_mut_(tree, $.copy(exports.RIGHT), $c, [$p[0]]);
}
...
function traverse_successor_mut_(tree, key, parent_index, $c, $p) {
    return traverse_mut_(tree, $.copy(key), $.copy(parent_index), $.copy(exports.RIGHT), $c, [$p[0]]);
}
...
function traverse_successor_pop_mut_(tree, key, parent_index, child_index, n_outer_nodes, $c, $p) {
    return traverse_pop_mut_(tree, $.copy(key), $.copy(parent_index), $.copy(child_index), $.copy(n_outer_nodes), $.copy(exports.RIGHT), $c, [$p[0]]);
}
...
function traverse_target_child_index_(tree, key, parent_index, direction, $c, $p) {
    ...
    while ((direction != is_set_($.copy(key), $.copy(parent.critical_bit), $c))) {
        {
            parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent.parent_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]);
        }
    }
    if ((direction == $.copy(exports.LEFT))) {
        temp$1 = $.copy(parent.left_child_index);
    ...
    while (!is_outer_node_($.copy(child_index), $c)) {
        {
            if ((direction == $.copy(exports.LEFT))) {
                temp$2 = $.copy(Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).right_child_index);
            }
            else {
                temp$2 = $.copy(Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_index), $c, [new move_to_ts_2.SimpleStructTag(InnerNode)]).left_child_index);
            }
            child_index = temp$2;
        }
    }
...
function loadParsers(repo) {
    repo.addParser("0x91f6206a9ee4f969adf1e987b11a6a7ff4848fdb63b90f05ecd8a1a73b0e3efa::critbit::CritBitTree", CritBitTree.CritBitTreeParser);
    repo.addParser("0x91f6206a9ee4f969adf1e987b11a6a7ff4848fdb63b90f05ecd8a1a73b0e3efa::critbit::InnerNode", InnerNode.InnerNodeParser);
    repo.addParser("0x91f6206a9ee4f969adf1e987b11a6a7ff4848fdb63b90f05ecd8a1a73b0e3efa::critbit::OuterNode", OuterNode.OuterNodeParser);
}
