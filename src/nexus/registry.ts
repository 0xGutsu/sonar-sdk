...
export const E_NOT_NEXUS: U64 = u64("0");
...
export function destroy_custodian_capability_test_(
...
export function get_custodian_capability_test_(
  custodian_id: U64,
  $c: AptosDataCache
): CustodianCapability {
...
export function init_registry_(account: HexString, $c: AptosDataCache): void {
  if (
    ...
    throw $.abortCode($.copy(E_NOT_NEXUS));
  }
  if (
    !!$c.exists(
      new SimpleStructTag(Registry),
    ...
    new Registry(
      {
        ...
        markets: Stdlib.Vector.empty_($c, [new SimpleStructTag(MarketInfo)]),
        n_custodians: u64("0"),
      },
      new SimpleStructTag(Registry)
    )
...
export function is_registered_trading_pair_(
  ...
  return Stdlib.Table.contains_(
    ...
    $c,
    [new SimpleStructTag(TradingPairInfo), AtomicTypeTag.Address]
  );
}
...
export function n_markets_($c: AptosDataCache): U64 {
  if (
    !$c.exists(
      new SimpleStructTag(Registry),
      new HexString(
...
export function register_market_internal_(
  ...
  if (pure_coin) {
    ...
    if (
      !$.copy(generic_asset_transfer_custodian_id).eq($.copy(PURE_COIN_PAIR))
    ) {
...
export function set_registered_custodian_test_(
  custodian_id: U64,
  $c: AptosDataCache
): void {
  $c.borrow_global_mut<Registry>(
    new SimpleStructTag(Registry),
    new HexString(
      "0x91f6206a9ee4f969adf1e987b11a6a7ff4848fdb63b90f05ecd8a1a73b0e3efa"
    )
...
export class App {
  ...
  async init_registry(_account: AptosAccount, _maxGas = 1000, _isJSON = false) {
