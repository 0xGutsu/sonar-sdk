...
export class AllowedValidators 
{
  ...
  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, AllowedValidators, typeParams);
...
export class StakePool 
{
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.initialize_validator_events = proto['initialize_validator_events'] as Event.EventHandle;
...
export function initialize_owner_ (
  ...
  temp$9 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(SetOperatorEvent)]);
  temp$10 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(AddStakeEvent)]);
  temp$11 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(ReactivateStakeEvent)]);
  temp$12 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(RotateConsensusKeyEvent)]);
  temp$13 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(UpdateNetworkAndFullnodeAddressesEvent)]);
  temp$14 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(IncreaseLockupEvent)]);
  temp$15 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(JoinValidatorSetEvent)]);
  temp$16 = Account.new_event_handle_(owner, $c, [new SimpleStructTag(DistributeRewardsEvent)]);
...
export function rotate_consensus_key_ (
  ...
  validator_info = $c.borrow_global_mut<ValidatorConfig>(new SimpleStructTag(ValidatorConfig), $.copy(pool_address));
