...
export class AccountMap 
{
  ...
  static structName: string = "AccountMap";
...
export class EmployeeAccountMap 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "EmployeeAccountMap";
  static typeParameters: TypeParamDeclType[] = [

  ];
  static fields: FieldDeclType[] = [
  { name: "accounts", typeTag: new VectorTag(AtomicTypeTag.Address) },
  { name: "validator", typeTag: new StructTag(new HexString("0x1"), "genesis", "ValidatorConfigurationWithCommission", []) },
  { name: "vesting_schedule_numerator", typeTag: new VectorTag(AtomicTypeTag.U64) },
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.vesting_schedule_numerator = proto['vesting_schedule_numerator'] as U64[];
    this.vesting_schedule_denominator = proto['vesting_schedule_denominator'] as U64;
...
export class ValidatorConfigurationWithCommission 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "ValidatorConfigurationWithCommission";
  static typeParameters: TypeParamDeclType[] = [

  ];
  static fields: FieldDeclType[] = [
  { name: "validator_config", typeTag: new StructTag(new HexString("0x1"), "genesis", "ValidatorConfiguration", []) },
  ...
  static getTag(): StructTag {
    return new StructTag(moduleAddress, moduleName, "ValidatorConfigurationWithCommission", []);
  }
  async loadFullState(app: $.AppType) {
...
export function create_account_ (
  ...
  if (Account.exists_at_($.copy(account_address), $c)) {
    ...
    Coin.register_(account, $c, [new StructTag(new HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
...
export function create_accounts_ (
  ...
  i = u64("0");
  num_accounts = Vector.length_(accounts, $c, [new SimpleStructTag(AccountMap)]);
  unique_accounts = Vector.empty_($c, [AtomicTypeTag.Address]);
  while (($.copy(i)).lt($.copy(num_accounts))) {
    {
      account_map = Vector.borrow_(accounts, $.copy(i), $c, [new SimpleStructTag(AccountMap)]);
      if (!!Vector.contains_(unique_accounts, account_map.account_address, $c, [AtomicTypeTag.Address])) {
        throw $.abortCode(Error.already_exists_($.copy(EDUPLICATE_ACCOUNT), $c));
      }
      Vector.push_back_(unique_accounts, $.copy(account_map.account_address), $c, [AtomicTypeTag.Address]);
...
export function create_employee_validators_ (
  ...
  i = u64("0");
  num_employee_groups = Vector.length_(employees, $c, [new SimpleStructTag(EmployeeAccountMap)]);
  unique_accounts = Vector.empty_($c, [AtomicTypeTag.Address]);
  while (($.copy(i)).lt($.copy(num_employee_groups))) {
    {
      j = u64("0");
      employee_group = Vector.borrow_(employees, $.copy(i), $c, [new SimpleStructTag(EmployeeAccountMap)]);
      num_employees_in_group = Vector.length_(employee_group.accounts, $c, [AtomicTypeTag.Address]);
      buy_ins = Simple_map.create_($c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "coin", "Coin", [new StructTag(new HexString("0x1"), "aptos_coin", "AptosCoin", [])])]);
      while (($.copy(j)).lt($.copy(num_employees_in_group))) {
        {
          account = Vector.borrow_(employee_group.accounts, $.copy(j), $c, [AtomicTypeTag.Address]);
          if (!!Vector.contains_(unique_accounts, account, $c, [AtomicTypeTag.Address])) {
            throw $.abortCode(Error.already_exists_($.copy(EDUPLICATE_ACCOUNT), $c));
          }
          Vector.push_back_(unique_accounts, $.copy(account), $c, [AtomicTypeTag.Address]);
          employee = create_signer_($.copy(account), $c);
          total = Coin.balance_($.copy(account), $c, [new StructTag(new HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
          coins = Coin.withdraw_(employee, $.copy(total), $c, [new StructTag(new HexString("0x1"), "aptos_coin", "AptosCoin", [])]);
          Simple_map.add_(buy_ins, $.copy(account), coins, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "coin", "Coin", [new StructTag(new HexString("0x1"), "aptos_coin", "AptosCoin", [])])]);
          j = ($.copy(j)).add(u64("1"));
        }

      }j__1 = u64("0");
      num_vesting_events = Vector.length_(employee_group.vesting_schedule_numerator, $c, [AtomicTypeTag.U64]);
      schedule = Vector.empty_($c, [new StructTag(new HexString("0x1"), "fixed_point32", "FixedPoint32", [])]);
      while (($.copy(j__1)).lt($.copy(num_vesting_events))) {
        {
          numerator = Vector.borrow_(employee_group.vesting_schedule_numerator, $.copy(j__1), $c, [AtomicTypeTag.U64]);
          event = Fixed_point32.create_from_rational_($.copy(numerator), $.copy(employee_group.vesting_schedule_denominator), $c);
          Vector.push_back_(schedule, $.copy(event), $c, [new StructTag(new HexString("0x1"), "fixed_point32", "FixedPoint32", [])]);
      ...
      if ((($.copy(employee_group.beneficiary_resetter)).hex() !== (new HexString("0x0")).hex())) {
...
export function create_initialize_validators_ (
  ...
  validators_with_commission = Vector.empty_($c, [new SimpleStructTag(ValidatorConfigurationWithCommission)]);
  while (($.copy(i)).lt($.copy(num_validators))) {
    {
      validator_with_commission = new ValidatorConfigurationWithCommission({ validator_config: Vector.pop_back_(validators, $c, [new SimpleStructTag(ValidatorConfiguration)]), commission_percentage: u64("0"), join_during_genesis: true }, new SimpleStructTag(ValidatorConfigurationWithCommission));
      Vector.push_back_(validators_with_commission, $.copy(validator_with_commission), $c, [new SimpleStructTag(ValidatorConfigurationWithCommission)]);
      i = ($.copy(i)).add(u64("1"));
    }
...
export function create_initialize_validators_with_commission_ (
  ...
  i = u64("0");
  num_validators = Vector.length_(validators, $c, [new SimpleStructTag(ValidatorConfigurationWithCommission)]);
  while (($.copy(i)).lt($.copy(num_validators))) {
    {
      validator = Vector.borrow_(validators, $.copy(i), $c, [new SimpleStructTag(ValidatorConfigurationWithCommission)]);
