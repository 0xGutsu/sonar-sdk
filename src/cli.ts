...
const assets_init_all_coin_types = async () => {
...
const assets_mint = async (CoinType: string, amount: string) => {
  const {client, account} = readConfig(program);
...
const assets_mint_and_deposit = async (CoinType: string, amount: string) => {
  const {client, account} = readConfig(program);
  const CoinType_ = parseTypeTagOrThrow(CoinType);
  const amount_ = u64(amount);
...
const market_register_market_generic = async (BaseType: string, QuoteType: string, lot_size: string, tick_size: string, generic_asset_transfer_custodian_id: string) => {
  const {client, account} = readConfig(program);
  const BaseType_ = parseTypeTagOrThrow(BaseType);
  const QuoteType_ = parseTypeTagOrThrow(QuoteType);
  const lot_size_ = u64(lot_size);
  const tick_size_ = u64(tick_size);
  const generic_asset_transfer_custodian_id_ = u64(generic_asset_transfer_custodian_id);
